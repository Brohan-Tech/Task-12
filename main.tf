...
resource "aws_lb_target_group" "rohana_strapi_tg_blue" {
  name        = "rohana-strapi-tg-blue"
  port        = 1337
  protocol    = "HTTP"
...
resource "aws_lb_target_group" "rohana_strapi_tg_green" {
  name        = "rohana-strapi-tg-green"
  port        = 1337
  protocol    = "HTTP"
  vpc_id      = "vpc-06ba36bca6b59f95e"
  target_type = "ip"
...
resource "aws_codedeploy_deployment_group" "rohana_strapi_dg" {
  ...
  ecs_service {
    cluster_name = aws_ecs_cluster.rohana_strapi_cluster.name
